...
_TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "properties": {
            ...
            "max_tilt_deg": {"type": "number", "description": "Maximum out-of-plane spore tilt in degrees for scene=spores."},
            "stacking": {"type": "number", "description": "0-1 tendency for overlapping spores to sit on top of existing spores instead of all sharing the substrate height."},
            "debris_clumps": {"type": "integer", "description": "Number of granular debris aggregates to add for scene=spores."},
            "debris_grain_nm": {"type": "number", "description": "Typical debris granule diameter in nm."},
            "debris_spread_nm": {"type": "number", "description": "Spatial spread of each debris aggregate in nm."},
            ...
            "shadowing": {"type": "number", "description": "0-1 detector occlusion by raised relief; higher values add stronger cast shadows behind topography."},
            "read_noise_e": {"type": "number", "description": "Read noise in electrons."},
...
class LLMAgent(QThread):
    ...
    def _dispatch_tool(self, name: str, params: dict) -> str:
        ...
        if name in _NEEDS_CONFIRM:
            ...
            # Result-returning actions block until their handler reports a real
            # outcome (count / path / verdict) instead of a blind "dispatched".
            is_result = name in _RESULT_TOOLS
            ...
            if is_result:
                res = (self._context.wait_for_action_result(timeout=120.0)
                       if self._context is not None else None)
                return res or (f"'{name}' was dispatched but returned no result in time — "
                               "tell the user to check the relevant panel; do not assert it succeeded.")
            if is_meas:
                return (
