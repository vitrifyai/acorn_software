...
| Group | Contents |
|-------|---------|
| `entry/instrument/` | Microscope name, accelerating voltage, magnification |
| `entry/sample/` | Sample name |
| `entry/images/<filename>/data/` | Image pixel array (gzip-compressed) with calibrated x/y axes in nm |
| `entry/images/<filename>/annotations/` | ROI polygons, areas (nm²), distances, labels |
| `entry/measurements/` | Full particle measurement table: ECD, Feret diameter, area, circularity, aspect ratio — all with `units` attributes |
...
The file is written to `acorn_measurements/<folder_name>_acorn.nxs` in the same
directory as your images.

### Opening the file

The `.nxs` file can be opened with any NeXus-aware tool:
- **h5web** — browser-based HDF5 viewer
